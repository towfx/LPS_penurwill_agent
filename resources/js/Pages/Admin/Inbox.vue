...
function switchTab(key) {
  ...
  router.get('/admin/inbox', { tab: key }, { preserveState: false })
}

function markRead(id) {
  router.post(`/admin/inbox/${id}/read`, {}, { preserveScroll: true })
}

function markAllRead() {
  router.post('/admin/inbox/read-all', {}, { preserveScroll: true })
}

function changePage(page) {
  router.get('/admin/inbox', { tab: currentTab.value, page }, { preserveScroll: true })
}
...
<template>
  <div>
    <PageHeader
      title="Inbox"
      description="System notifications and important messages."
      :breadcrumbs="[{ label: 'Admin', href: '/admin/dashboard' }, { label: 'Inbox' }]"
    >
      <template #actions>
        <Button
