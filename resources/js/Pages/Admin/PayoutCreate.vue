<template>
  <div>
    <PageHeader
      ...
      :breadcrumbs="[{ label: 'Admin', href: '/admin/dashboard' }, { label: 'Commissions', href: '/admin/commissions/list' }, { label: 'Create Payout' }]"
    >
      <template #actions>
    ...
    <form @submit.prevent="submitForm">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-stone-200 p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div class="text-center">
            <div class="text-2xl font-bold text-forest-dark">{{ approvedSummary.total_sales }}</div>
            <div class="text-sm text-stone-500">Selected Sales</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-forest-dark">
              {{ formatCurrency('RM', approvedSummary.total_commission) }}
            </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-stone-200 p-6 mb-6">
        ...
        <div class="mb-6">
          <FormField label="Payout Amount">
            <Input v-model="form.amount" type="number" :placeholder="formatCurrency('RM', approvedSummary.total_commission)" />
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead class="bg-stone-50">
              <tr>
                <th class="px-6 py-3 text-center text-xs font-medium text-stone-500 uppercase tracking-wider">Select</th>
            ...
            <tbody class="bg-white divide-y divide-stone-200">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  ...
                  <span v-if="commission.is_reversal" class="ml-1 text-xs text-accent-red">↩</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm">
...
const submitForm = () => {
  ...
  form.post('/admin/payout/store')
}

onMounted(() => {
  // Initialize with all commissions selected
  form.commission_ids = props.commissions.map(c => c.id)
